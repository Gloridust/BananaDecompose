...
左栏底部的**一键评测**把七种方案用同一条提示词挨个跑一遍，每组单独存一条运行记录。
...
跑完点顶部的「查看对比表」，或者去 `/history` 展开评测组。
...
### 6. 历史记录：localStorage 索引 + IndexedDB 数据
...
`/history` 有两种对比方式：按**评测组**展开完整指标表，或者从全部运行里任选最多 3 条并排比。两种都带缩略图、耗时、成本、每一步明细和所有中间产物。
